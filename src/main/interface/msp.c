...
static bool mspProcessOutCommand(uint8_t cmdMSP, sbuf_t *dst)
{
    ...
    case MSP_LED_COLORS:
        for (int i = 0; i < LED_CONFIGURABLE_COLOR_COUNT; i++) {
           	const hsvColor_t *color = &ledStripProfiles(systemConfig()->activeLedStripProfile)->colors[i];
    ...
    case MSP_LED_STRIP_MODECOLOR:
        for (int i = 0; i < LED_MODE_COUNT; i++) {
            for (int j = 0; j < LED_DIRECTION_COUNT; j++) {
                sbufWriteU8(dst, i);
                sbufWriteU8(dst, j);
            	sbufWriteU8(dst, ledStripProfiles(systemConfig()->activeLedStripProfile)->modeColors[i].color[j]);
			}
        }
        ...
        for (int j = 0; j < LED_SPECIAL_COLOR_COUNT; j++) {
            ...
            sbufWriteU8(dst, j);
        	sbufWriteU8(dst, ledStripProfiles(systemConfig()->activeLedStripProfile)->specialColors.color[j]);
		}

        sbufWriteU8(dst, LED_AUX_CHANNEL);
        sbufWriteU8(dst, 0);
        sbufWriteU8(dst, ledStripProfiles(systemConfig()->activeLedStripProfile)->ledstrip_aux_channel);
		break;
#endif
...
static mspResult_e mspProcessInCommand(uint8_t cmdMSP, sbuf_t *src)
{
    ...
    case MSP_SET_LED_COLORS:
        for (int i = 0; i < LED_CONFIGURABLE_COLOR_COUNT; i++) {
           	hsvColor_t *color = &ledStripProfilesMutable(systemConfig()->activeLedStripProfile)->colors[i];
    ...
    case MSP_SET_LED_STRIP_CONFIG:
        {
            ...
            ledConfig_t *ledConfig = &ledStripProfilesMutable(systemConfig()->activeLedStripProfile)->ledConfigs[i];
